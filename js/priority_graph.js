jQuery(document).ready(function() {
    ...
    for (var a = 0; a < task_complexities.length; a++) {
      var c = task_complexities[a];

      if (!Number.isInteger(c)) {
        task_complexities[a] = convertSizeToNumber(c);
      }
    }
    for (var a = 0; a < task_values.length; a++) {
      var v = task_values[a];

      if (!Number.isInteger(v)) {
        task_values[a] = convertSizeToNumber(v);
      }
    }
